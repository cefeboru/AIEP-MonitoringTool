	...
	<bbNG:pageHeader>
		...
		<bbNG:form id="AdminForm" method = "POST" action = "${actionUrl}" enctype="application/x-www-form-urlencoded">
			<bbNG:dataCollection>
				...
				<bbNG:step id="StudentReport" title="Control de Estudiantes SP" instructions= ".">
					...
					<img src="/webapps/lnoh-AIEPMTOOL-BBLEARN/Images/xls.png" style="width: 2%;">&nbsp;<label id="DownloadReport" style="font-size: 84%;font-Weight: bold;cursor: pointer;">Descargar Listado</label>&nbsp;&nbsp;&nbsp;&nbsp;
						...
						<input type="button" id ="Emailbtn" value="Enviar">
					</div>
...
$( document ).ready( function() {
	...
	var ManageWindow = null;
	    	...
	    	$.ajax({
		           ...
		           success:function(result){
			            ...
			            if(result.indexOf("<table ") != -1){
			            	...
			            	document.getElementById("grid-basic-footer").parentNode.parentNode.appendChild(document.getElementById("grid-basic-footer"));
			            	document.getElementById("grid-basic-header").parentNode.parentNode.insertBefore(document.getElementById("grid-basic-header"),document.getElementById("grid-basic-header").parentNode.parentNode.children[0]);

			            	var ActionBtns = document.getElementsByClassName("dropdown btn-group");

			            	ActionBtns[0].id = "MaxRowNum";
			            	ActionBtns[1].id = "HideColumns";
			            	$("#HideColumns").remove();
			            	$("#MaxRowNum").on("mouseover",function(){if(this.className == "dropdown btn-group"){this.className = "dropdown btn-group open";}});
			        	    $("#MaxRowNum").on("mouseout",function(){this.className = "dropdown btn-group";});
			        	    document.getElementById("TableData").appendChild(document.getElementById("grid-basic-footer"));
			        	    */
			            }
		       		},
		       		error: function (jqXHR, exception) {
		       	        ...
		       	        document.getElementById("StudentData").innerHTML +=  "</br>" + msg;
		       	    }
		           });
	    };
	    	...
	    	$.ajax({
		           ...
		           success:function(result){
						...
						if(result.indexOf("<table") != -1){
							...
							link.setAttribute("download", "InformeMonitoreo.csv");
							link.click();
		...
		document.getElementById("Emailbtn").onclick = function(){
	    	...
	    	var SelectedFormat = document.getElementById("EmailFormat").value;
	    	var Emails = "";
	    	var Ids = "";
	    	var Names = "";
	    	var Status = "";
	    	
	    	$(':checkbox:checked').each(function(){Emails+=this.value+","; Ids+=this.id.replace("Chk","") + ","; Names +=(document.getElementById("First" + this.id.replace("Chk","")).innerHTML + " " + document.getElementById("Last" + this.id.replace("Chk","")).innerHTML + ","); Status+=document.getElementById("Status" + this.id.replace("Chk","")).children[0].title + ",";});
	    	...
	    	if(Emails != ""){
	    		...
	    		$.ajax({
			           ...
			           success:function(result){
			        	   ...
			        	   $(':checkbox:checked').each(function(){$("#Sent" + this.id.replace("Chk",""))[0].src = "/webapps/lnoh-AIEPMTOOL-BBLEARN/Images/chk.png";});
			        	   $(':checkbox:checked').each(function(){this.checked = false;});
			        	   alertify.success("Se enviaron los correos exitosamente.",1500);
			       		},
			       		error: function (jqXHR, exception) {
			       	        ...
			       	        if (jqXHR.status === 0) {
			       	     	...
			       	     	alertify.error("Hubo un error al enviar los correos, intente de nuevo." + msg,2000);
			       	    }
	           });
	    	}
	    	else{
	    		
	    		alertify.error("Seleccione al menos un estudiante.",3000);
	    	}
	    };
	    ...
	    function Management(e){
	    	...
	    	var Status = document.getElementById("Status" + Id).innerHTML.replace("60","35");

	    	Status = Status + "<label id='StatusText'>" + document.getElementById("Status" + Id).children[0].title + "</label>";
	    ...
	    function Record(e){
	    	...
	    	var Interval1 = setInterval(function(){
	    		...
	    		if(RecordWindow.document.getElementById("StuName") != null &&
	    			...
	    			RecordWindow.document.getElementById("StuRut").innerHTML = "";
	    			RecordWindow.document.getElementById("StuPhone").innerHTML = "";
	    			RecordWindow.document.getElementById("StuEmail").innerHTML = Email;
	    			RecordWindow.document.getElementById("StuHeadquarter").innerHTML = document.getElementById("Headquarters").value;
